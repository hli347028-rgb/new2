export default {
  common: {
    connectWallet: '連接錢包',
    selectLanguage: '選擇語言',
    noData: '暫無數據',
    confirm: '確定',
    prompt: '提示',
    comingSoon: '暫未開放',
    submit: '提交',
    enterInviteCode: '請輸入邀請碼',
    contractVerifying: '合約驗證中',
    walletConnecting: '錢包連接中',
    insufficientBalance: '餘額不足',
    quantity: '數量',
    copiedToClipboard: '內容已複製到剪貼板',
    authorizing: '授權中'
  },
  index: {
    welcomeSubtitle: '全球首個 AI 溯源應用鏈',
    welcomeDescription: '數字文明的價值溯源網絡',
    welcomeDescription2: '全球首個AI公鏈、支付、鏈遊、鏈上商城為一體的超級應用',
    projectInfoTitle1: '一、結合AI人工智能與技術優勢',
    totalSupply: '代幣總發行量1億枚',
    nodeOutput: '節點輸出：',
    blockInterval: '塊輸出間隔：3 seconds/block',
    blocksPerDay: '每24小時區塊：22300',
    scenariosTitle: '二、實用場景',
    chainLaunch: 'Taurus公鏈：預計2027年5月代碼正式開源',
    miningLaunch: 'Taurus挖礦：2026年{month}月中旬上線',
    paymentLaunch: '鏈上支付：2026年10月上線',
    gameBeta: '鏈上遊戲：2026年11月份上線',
    mallLaunch: '鏈上商城：2026年12月份上線',
    techTitle: '三、技術架構',
    techDesc: '分佈式存儲：我們的平台匯聚了眾多開發者與企業，促進了AI智能領域專業人士、企業家及合作夥伴間的緊密合作，共同推動科技',
    startNow: '立即開始',
    idoRecruitPlan: 'IDO招募計劃',
    idoSubtitle: '成為Taurus IDO，享受提現手續費分紅。贈送社區等級，代幣空投等權益。',
    idoRights: 'IDO權益',
    limited: '限量{count}份',
    usdtPerShare: 'USDT/份',
    accountLimit: '單個帳號限購 {count} 份',
    featureTitles: ['聚合支付', '鏈遊', 'RWA'],
    benefit_txfee: '交易稅費分紅',
    benefit_txfeeDesc: '獲得交易買賣稅各1%收益',
    benefit_profit: '盈利稅費分紅',
    benefit_profitDesc: '獲得5%的盈利稅費分紅',
    benefit_referral: '推薦獎勵',
    benefit_referralDesc: '推薦IDO可以獲得10%的直推獎勵',
    benefit_airdrop: '代幣空投',
    benefit_airdropDesc: '獲得1050枚FDA代幣空投，逐步釋放',
    benefit_maker: '做市商分紅',
    benefit_makerDesc: '獲得3%做市商利潤加權分紅',
    becomeIdo: '立即成為IDO',
    nftRequirement: '需10個IDO獲得NFT',
    nftDesc: '成功推薦十份IDO即可獲得一張限量NFT!享受做市商2%全球利潤分紅，NFT總發行300張',
    nftProgress: '還需10份IDO獲取NFT',
    nftRights: 'NFT權益',
    partners: '合作機構'
  },
  community: {
    level: '等級:',
    superiorAddress: '上級邀請地址',
    myInviteLink: '我的邀請鏈接',
    evangelists: '佈道人數',
    activeUsers: '有效用戶',
    communitySize: '社區人數',
    totalIncome: '累計收益',
    communityPerformance: '社區業績',
    totalPerformance: '總業績',
    regionalPerformance: '大區業績',
    smallPerformance: '小區業績',
    directInviteData: '直接邀請數據',
    teamIdoTotal: '團隊IDO總量',
    personalRedeemed: '個人已贖回業績',
    teamRedeemed: '團隊已贖回業績',
    evangelistList: '佈道列表',
    walletAddress: '錢包地址',
    userLevel: '用戶等級',
    personalPerformance: '個人業績',
    teamPerformance: '團隊業績',
    directReferralIncome: '直推收益',
    rewardRecord: '獎勵記錄',
    amount: '數量',
    generation: '代數',
    reward: '獎勵',
    time: '時間'
  },
  mine: {
    tokenAirdrop: '獎勵結算',
    claimableAmount: '可領取金額',
    pendingAmount: '待釋放金額',
    claimedAmount: '已領取金額',
    remainingAmount: '剩餘金額',
    unexitedAmount: '未出局金額',
    rewardDetail: '收益明細',
    staticIncome: '靜態收益',
    generationReward: '代數獎勵',
    communityBaseReward: '社區基礎獎',
    equalBonus: '平級分紅',
    totalNodes: '訂單數',
    releaseCountdown: '釋放倒計時',
    none: '無',
    claimAirdrop: '提現',
    claimRecord: '領取記錄',
    time: '時間',
    amount: '金額'
  },
  pledge: {
    currentAssets: '當前資產',
    startMining: '開始挖礦',
    pledge: '質押',
    selectCycle: '選擇質押週期',
    days: '天',
    dailyCompound: '日複利',
    availableBalance: '可用餘額：',
    enterPledgeAmount: '請輸入質押金額',
    max: 'MAX',
    minPledge: '最低質押：',
    maxPledge: '最高質押：',
    estimatedProfit: '預計收益:',
    pledgeNow: '立即質押'
  },
  node: {
    pageTitle: '節點認購',
    pageSubtitle: '選擇適合您的節點檔位',
    subscribed: '已認購',
    statusAvailable: '可認購',
    statusFull: '已售罄',
    statusComing: '即將開放',
    subscribeNow: '立即認購',
    subscribeSuccess: '認購成功',
    subscribeFailed: '認購失敗',
    orderList: '認購記錄',
    amount: '金額',
    status: '狀態',
    time: '時間',
    statusEarning: '收益中',
    statusCompleted: '已完成',
    statusPending: '待處理',
    customAmount: '自訂金額 (≥ {min} USDT)',
    minPlaceholder: '最低 {min}',
    minAmountTip: '最低認購金額為 {min} USDT',
    walletBalance: '帳戶餘額',
    rechargeTip: '無金額限制，任意金額均可充值',
    rechargeSuccess: '充值成功',
    rechargeFailed: '充值失敗',
    rechargeAmountInvalid: '請輸入大於 0 的充值金額'
  },
  recharge: {
    recharge: '充值',
    rechargeRecord: '充值記錄',
    currentBalance: '當前餘額',
    enterAmount: '請輸入數量',
    minRechargeAmount: '最低充值金額',
    date: '日期',
    amount: '金額'
  },
  tab: {
    home: '首頁',
    mining: '挖礦',
    community: '社區',
    mine: '我的',
    nodeSubscription: '節點認購',
    pledgeMining: '質押挖礦',
    myTeam: '我的團隊',
    myAssets: '我的資產',
    internationalPayment: '國際支付',
    chainGames: '鏈上遊戲',
    chainGamesBeta: '內測中2026年11月份上線',
    chainMall: '鏈上商城',
    taurusChain: 'Taurus公鏈'
  },
  route: {
    community: '社區',
    pledge: '質押',
    mine: '我的'
  },
  advantage: {
    title: '優勢',
    revenueTag: '高效收益',
    revenueDesc: '優化的收益算法，提供行業領先的質押回報率',
    transparentTag: '透明公開',
    transparentDesc: '所有交易和收益數據上鏈可查，完全透明公開',
    securityTag: '安全可靠',
    securityDesc: '基於BSC鏈的智能合約確保資產安全'
  },
  wallet: {
    installMetaMask: '請安裝 MetaMask 錢包',
    connectFailed: '錢包連接失敗',
    fetchBalanceFailed: '獲取餘額失敗'
  },
  announcement: {
    title: '公告',
    confirm: '確認',
    noRemind: '不再提醒',
    content: 'Taurus 正式上線啦',
    gotIt: '我知道了'
  }
}
